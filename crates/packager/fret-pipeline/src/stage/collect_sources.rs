//! Discovers all .zo source files via iterative directory
//! traversal (manual stack, no recursion).
...
/// Stage that collects all .zo source files from the source
/// directory.
...
impl Stage for CollectSources {
  fn execute(&self, ctx: &mut BuildContext) -> Result<(), StageError> {
    ...
    // Deterministic build order regardless of filesystem.
    ...
    for entry in entries {
      ...
      if file_type.is_dir() {
        if let Some(name) = path.file_name() {
          ...
          // Skip hidden dirs + common build artifacts.
        ...
        && path.extension().is_some_and(|ext| ext == "zo")
      {
        files.push(path);
      }
    }
