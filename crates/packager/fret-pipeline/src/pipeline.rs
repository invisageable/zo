...
use fret_types::{BuildContext, Stage, StageError, Target};

use std::path::PathBuf;
use std::time::Instant;

/// Executes stages in sequence, transforming a project path
...
impl Pipeline {
  /// Simple Mode: maximum compilation speed, single project,
  /// no incremental builds.
  pub fn simple_mode() -> Self {
    let stages: Vec<Box<dyn Stage>> = vec![
      Box::new(LoadConfig),
      Box::new(CollectSources),
      Box::new(ResolveDependencies),
  ...
  /// Execute the entire pipeline, transforming raw project
  /// path into compiled binary. Returns the output binary
  /// path on success.
  pub fn execute(
    &self,
  ...
  /// Execute the entire pipeline with an optional target
  /// override. Returns the output binary path on success.
  pub fn execute_with_target(
    ...
    let config = fret_parser::parse_config(&config_content)
