//! Recursive descent parser for fret.oz configuration files.
//!
//! Transforms a token stream into [`ProjectConfig`] with
//! source-located error messages.

use fret_tokenizer::Tokenizer;
use fret_tokens::{Token, TokenKind};
use fret_types::{ProjectConfig, StageError, Version};
...
pub struct ParseError {
  ...
  pub column: usize,
  /// Source line with a `^` pointer.
  pub snippet: String,
}

/// Parser for fret.oz configuration files.
pub struct Parser<'src> {
  /// The tokenizer providing tokens
...
impl<'src> Parser<'src> {
  ...
  pub fn parse(mut self) -> Result<ProjectConfig, StageError> {
    ...
    // Optional namespace segments: @pack:zo:release
  ...
  fn consume_string(&mut self, error_msg: &str) -> Result<String, StageError> {
    ...
    // Strip surrounding quotes before unescaping.
...
/// Convenience entry point: parse fret.oz source into a
/// [`ProjectConfig`].
