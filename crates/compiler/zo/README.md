...
- [ ] meta-language — *`#dom`, `#run` (directives)*.
- [x] templating syntax — *like the abandoned `ecmascript 4`*.
- [ ] build native app — *`gpu` (egui) and `wasm` (web-sys)*.
- [ ] safe concurrency model — *actor model erlang-like*.
- [x] fast `compilation-time` — *insanely faster, usain is jealous*.
- [ ] powerfull `tools` — *native repl, code editor, packager, etc*.
