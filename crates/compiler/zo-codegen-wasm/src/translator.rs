...
use zo_ast::ast::{
  ...
  LitKind, Load, Stmt, StmtKind, Struct, TyAlias, UnOp, UnOpKind, Var,
};
...
impl<'ast> Translator<'ast> {
  ...
  fn translate_item(&mut self, item: &Item) -> Result<()> {
    match &item.kind {
      ...
      ItemKind::Struct(structure) => self.translate_item_struct(structure),
  ...
  fn translate_item_struct(&mut self, _structure: &Struct) -> Result<()> {
    todo!()
  }
