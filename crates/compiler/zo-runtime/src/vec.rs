...
//! `*mut ZoVec` itself never moves. `_zo_vec_free`
//! reclaims the box.
...
/// `Box::leak`-ed; the matching `_zo_vec_free` reclaims
/// it.
///
...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_new(
  _elem_kind: u8,
  elem_sz: usize,
  cap: usize,
) -> *mut ZoVec {
...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_push(
  vec: *mut ZoVec,
  val_ptr: *const u8,
) {
...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_pop(
  vec: *mut ZoVec,
  val_out: *mut u8,
) -> bool {
  let v = unsafe { &mut *vec };
...
/// As `_zo_vec_pop`.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_get(
  vec: *mut ZoVec,
  idx: usize,
  val_out: *mut u8,
) -> bool {
...
/// `elem_sz` readable bytes.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_set(
  vec: *mut ZoVec,
  idx: usize,
  val_ptr: *const u8,
) -> bool {
...
/// As `_zo_vec_pop`.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_remove(
  vec: *mut ZoVec,
  idx: usize,
  val_out: *mut u8,
) -> bool {
...
/// `vec` must be live.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_len(vec: *mut ZoVec) -> usize {
  let v = unsafe { &*vec };
...
/// # Safety
///
/// `vec` must be a pointer from `_zo_vec_new` that
/// hasn't been freed.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_free(vec: *mut ZoVec) {
  if !vec.is_null() {
    unsafe {
...
/// `zo_map_show`: walk the live entries, format each
...
/// # Safety
///
/// `vec` must be a live pointer from `_zo_vec_new`.
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_vec_show(
  vec: *mut ZoVec,
  fd: usize,
  elem_fmt: u8,
) {
  ...
  for i in 0..v.len {
    ...
    // value-side treatment in `zo_map_show`).
...
mod tests {
  ...
  fn empty_vec_len_zero() {
    let v = unsafe { zo_vec_new(0, 8, 0) };

    assert_eq!(unsafe { zo_vec_len(v) }, 0);
  ...
  fn push_pop_round_trip() {
    ...
    for i in 0..5i64 {
      ...
      unsafe {
        zo_vec_push(v, buf.as_ptr());
      }
    }

    assert_eq!(unsafe { zo_vec_len(v) }, 5);

    let mut out = [0u8; 8];

    for expected in (0..5i64).rev() {
      let ok = unsafe { zo_vec_pop(v, out.as_mut_ptr()) };
  ...
  fn grows_past_initial_capacity() {
    ...
    assert_eq!(unsafe { zo_vec_len(v) }, 32);

    let mut out = [0u8; 8];

    for i in 0..32i64 {
      let ok = unsafe { zo_vec_get(v, i as usize, out.as_mut_ptr()) };
  ...
  fn get_set_round_trip() {
    ...
    let ok = unsafe { zo_vec_set(v, 1, new_buf.as_ptr()) };

    assert!(ok);

    let mut out = [0u8; 8];
    let ok = unsafe { zo_vec_get(v, 1, out.as_mut_ptr()) };

    assert!(ok);
    assert_eq!(i64::from_le_bytes(out), 999);

    let oob = unsafe { zo_vec_set(v, 99, new_buf.as_ptr()) };

    assert!(!oob);

    let oob = unsafe { zo_vec_get(v, 99, out.as_mut_ptr()) };
