...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_float_to_str(
  f: f64,
) -> *const u8 {
...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_bool_to_str(
  b: i64,
) -> *const u8 {
  static TRUE_STR: std::sync::OnceLock<usize> =
    std::sync::OnceLock::new();
  static FALSE_STR: std::sync::OnceLock<usize> =
    std::sync::OnceLock::new();
...
#[unsafe(no_mangle)]
pub unsafe extern "C-unwind" fn zo_char_to_str(
  c: u32,
) -> *const u8 {
...
pub unsafe extern "C-unwind" fn zo_str_multi_concat(
  ...
  let slices: &[*const u8] =
    unsafe { std::slice::from_raw_parts(ptrs, count) };
...
mod tests {
  ...
  fn multi_concat_single_element() {
    ...
    let ptrs = [src.as_ptr()];
    let result = unsafe {
      zo_str_multi_concat(1, ptrs.as_ptr())
    };
    let bytes = unsafe { str_bytes(result) };
  ...
  fn multi_concat_empty_strings() {
    ...
    let result = unsafe {
      zo_str_multi_concat(2, ptrs.as_ptr())
    };
  ...
  fn multi_concat_mixed_empty_and_content() {
    ...
    let result = unsafe {
      zo_str_multi_concat(3, ptrs.as_ptr())
    };
    let bytes = unsafe { str_bytes(result) };
