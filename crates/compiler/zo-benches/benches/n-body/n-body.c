/* n-body (benchmarksgame) — C, portable sqrt version (Christoph Bauer).
   The original SSE intrinsic version is x86-only and its rsqrt
   approximation diverges from the sqrt-based Go/Rust/zo ports; this
   plain version compiles on arm64 and matches them bit-for-bit. */
...
void advance(int nbodies, struct planet *b, double dt) {
  int i, j;
  for (i = 0; i < nbodies; i++) {
...
double energy(int nbodies, struct planet *b) {
  double e = 0.0;
  int i, j;
  for (i = 0; i < nbodies; i++) {
    e += 0.5 * b[i].mass *
...
void offset_momentum(int nbodies, struct planet *b) {
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < n; i++) advance(NBODIES, bodies, 0.01);
  printf("%.9f\n", energy(NBODIES, bodies));
  return 0;
}
