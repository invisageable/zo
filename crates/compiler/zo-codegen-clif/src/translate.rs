    ...
    match insn {
      ...
      Insn::ToStr { dst, src, src_ty } => {
        ...
        let result = if src_ty.0 == 4 {
          ...
          let fid = ensure_libc_func(tctx, name, |ptr_ty, cc| {
            let mut sig = ir::Signature::new(cc);

            sig.params.push(AbiParam::new(if is_float(*src_ty) {
              ir::types::F64
            } else {
              ir::types::I64
            }));
      ...
      Insn::StringFormat { dst, segments, .. } => {
        ...
        let count = builder.ins().iconst(ir::types::I64, segments.len() as i64);
        let fid =
          ensure_libc_func(tctx, "zo_str_multi_concat", |ptr_ty, cc| {
            let mut sig = ir::Signature::new(cc);
        ...
        let fref = tctx.module.declare_func_in_func(fid, builder.func);
        let call = builder.ins().call(fref, &[count, addr]);

        ctx.values.insert(*dst, builder.inst_results(call)[0]);
      }
