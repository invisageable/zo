...
impl<'mir> Translator<'mir> {
  ...
  fn infer_expr_loop(&mut self, _block: &ast::Block) -> Result<Value> {
    todo!()
  }
  ...
  fn infer_expr_for(&mut self, _for_loop: &ast::For) -> Result<Value> {
    todo!()
  }
  ...
  fn infer_expr_continue(&mut self) -> Result<Value> {
    todo!()
  }

  fn infer_expr_var(&mut self, var: &ast::Var) -> Result<Value> {
    self.translate_var(var)
  }
