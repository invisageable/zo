...
fn reconstruct(segments: &[InterpSegment], interner: &Interner) -> String {
...
fn adjacent_variables() {
  assert!(roundtrip("{x}{y}"), "adjacent variables failed roundtrip");
}
...
fn unmatched_open_brace() {
  assert!(roundtrip("{not_closed"), "unmatched brace failed roundtrip");
}
...
#[test]
fn closing_brace_splits_before_ident_check() {
  // `{a}}` — tokenizer scans `{` to first `}`, gets
  // `a` (valid ident), trailing `}` becomes literal.
  let segments = segments_for("{a}}");

  assert_eq!(segments.len(), 2);
  assert!(matches!(segments[0], InterpSegment::Variable(_)));
  assert!(matches!(segments[1], InterpSegment::Literal(_)));
}
