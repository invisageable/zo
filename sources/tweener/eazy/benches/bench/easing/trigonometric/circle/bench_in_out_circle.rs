...
pub fn in_out_circle(c: &mut Criterion) {
  ...
  group.bench_function("eazy", |b| {
    use eazy::Curve;
    use eazy::trigonometric::circle::InOutCircle;
  ...
  group.bench_function("bevy_tween", |b| {
    ...
    b.iter(|| {
      for num in nums.iter() {
        black_box(EaseKind::CircularInOut.sample(*num));
      }
    })
  });
  ...
  group.bench_function("easings", |b| {
    ...
    b.iter(|| {
      for num in nums.iter() {
        black_box(easings::circular_in_out(*num as f64));
      }
    })
  });
  ...
  group.bench_function("emath", |b| {
    ...
    b.iter(|| {
      for num in nums.iter() {
        black_box(easing::circular_in_out(*num));
      }
    })
  });
  ...
  group.bench_function("simple_easing2", |b| {
    ...
    b.iter(|| {
      for num in nums.iter() {
        black_box(simple_easing2::circ_in_out(*num));
      }
    })
  });
